...
public class MegaferiaView extends javax.swing.JFrame {
    ...
    // ====== MÉTODOS NUEVOS (punto 4 de la IA) ======
    private void actualizarTablaStands() {
        DefaultTableModel model = (DefaultTableModel) jTable3.getModel();
        model.setRowCount(0);

        Response<List<StandDTO>> response = standController.obtenerTodos();
        if (response != null && response.isSuccess() && response.getData() != null) {
            for (StandDTO s : response.getData()) {
                model.addRow(new Object[]{
                    s.getId(),
                    s.getPrecio(),
                    s.isComprado() ? "Sí" : "No",
                    "-" // columna de editoriales, se puede ajustar luego
                });
            }
        }
    }
    ...
    private void actualizarComboGerentes() {
        ...
        Response<List<PersonDTO>> response = personController.obtenerGerentes();
        if (response != null && response.isSuccess() && response.getData() != null) {
            for (PersonDTO p : response.getData()) {
                jComboBox1.addItem(p.getId() + " - " + p.getNombre() + " " + p.getApellido());
            }
        }
    }
